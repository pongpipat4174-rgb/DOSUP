...
-- =============================================
-- 7. ตาราง receiving_records (บันทึกรับเข้าสินค้า)
--    ย้ายจาก Google Sheet มาเก็บใน DB
-- =============================================
...
CREATE INDEX IF NOT EXISTS idx_receiving_date ON receiving_records(record_date);
CREATE INDEX IF NOT EXISTS idx_receiving_vendor ON receiving_records(vendor);
CREATE INDEX IF NOT EXISTS idx_receiving_product ON receiving_records(product_name);
CREATE INDEX IF NOT EXISTS idx_receiving_remark ON receiving_records USING gin (to_tsvector('simple', remark));

COMMENT ON TABLE receiving_records IS 'ตารางบันทึกรับเข้าสินค้า - ย้ายจาก Google Sheet';
COMMENT ON COLUMN receiving_records.record_date IS 'วันที่รับเข้า';
COMMENT ON COLUMN receiving_records.product_name IS 'ชื่อสินค้า';
COMMENT ON COLUMN receiving_records.total_boxes IS 'จำนวนลัง';
COMMENT ON COLUMN receiving_records.total_fractions IS 'จำนวนเศษ';
COMMENT ON COLUMN receiving_records.remark IS 'หมายเหตุ (เก็บเลข JO อ้างอิง)';
COMMENT ON COLUMN receiving_records.vendor IS 'แหล่งที่มา (จูน, CMI, บางปู)';

-- =============================================
-- 8. ตาราง delivery_plans (แผนจัดส่ง - View/Flatten)
